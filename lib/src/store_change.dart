...
> {
  ...
  StoreChange(State n, State p, Action<P> a) : next = n, prev = p, action = a;
}
...
typedef StoreChangeHandler<
  ...
  State extends Built<State, StateBuilder>,
  StateBuilder extends Builder<State, StateBuilder>
> = void Function(StoreChange<State, StateBuilder, P> storeChange);
...
> {
  ...
  void add<Payload>(
    ...
    _map[actionName.name] = (change) {
      handler(
        StoreChange<State, StateBuilder, Payload>(
          change.next,
          change.prev,
  ...
  void build(Store<State, StateBuilder, Actions> store) {
    _subscription = store.stream.listen((
      StoreChange<State, StateBuilder, dynamic> storeChange,
    ) {
